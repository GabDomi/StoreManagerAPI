...
namespace loja.services
{
    public class DepositoService
    {
        ...
        public async Task AddEstoqueAsync(int depositoId, int produtoId, int quantidade)
        {
          ...
          public async Task<DepositoProduto> GetQuantidadeProdutoNoDepositoAsync(int produtoId)
        {
        return await _dbContext.DepositoProdutos
            .FirstOrDefaultAsync(dp => dp.ProdutoId == produtoId);
        }

        public async Task UpdateQuantidadeProdutoNoDepositoAsync(int produtoId, int quantidade)
        {
        var produtoDeposito = await _dbContext.DepositoProdutos
            .FirstOrDefaultAsync(dp => dp.ProdutoId == produtoId);

        if (produtoDeposito != null)
        {
            produtoDeposito.Quantidade -= quantidade;
            await _dbContext.SaveChangesAsync();
        }
        }
    }
